...
writer = SummaryWriter('runs/MNIST_resnet18_2')
for epoch in range(epochs):
    ...
    for batch_idx,(input,target) in enumerate(train_loader):
        ...
        writer.add_scalar('Loss/train', running_loss, epoch*len(train_loader)+batch_idx)
...
with torch.no_grad():
    for batch_idx,(input,target) in enumerate(test_loader):
        ...
        writer.add_scalar('Loss/test', running_loss, epoch*len(test_loader)+batch_idx)
        writer.add_scalar('Accuracy/test', acc, epoch*len(test_loader)+batch_idx)
